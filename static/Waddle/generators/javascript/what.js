Blockly.JavaScript['what_waddle1'] = function (block) {
    return Math.trunc(new Date().getTime() / 1000) % 2 == 1 ? '"Waddle - 让Coco没有难做的控件"' : '"新年快乐(?)"';
};
...
Blockly.Blocks['game_tictactoe_board2'] = {
    init: function () {
        this.appendValueInput('1').setCheck('tpieces').setAlign(Blockly.ALIGN_CENTRE);
        this.appendValueInput('2').setCheck('tpieces').setAlign(Blockly.ALIGN_RIGHT);
        this.appendValueInput('3').setCheck('tpieces').setAlign(Blockly.ALIGN_RIGHT);
        this.setInputsInline(true);
...
Blockly.JavaScript['what_waddlerp'] = function () {
    return '"rp++;"\n';
};
