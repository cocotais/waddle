...
let js_save = function () {
    try {
        ...
        if (code_data.indexOf("isInvisibleWidget: true") === -1) {
...
let format_data = function (text, js) {
    return `<root><head><waddleversion>0</waddleversion><dataversion>0</dataversion><title></title></head><body><blocks>${text}</blocks><js>${js}</js></body></root>`;
};
